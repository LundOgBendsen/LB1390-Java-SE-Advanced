...
public class FileInfo
{
  ...
  {
    if (args.length == 0) throw new IllegalArgumentException("Der SKAL sendes en sti med til programmet");

    File file = new File(args[0]);
    if (!file.exists()) throw new IllegalArgumentException("Filen: " + args[0] + " eksisterer ikke!!");
    if (!file.isDirectory()) throw new IllegalArgumentException("Filen: " + args[0] + " er ikke en mappe!!");
    ...
    if (recursive)
    {
      ...
      System.out.println(file.getAbsolutePath() + " contains " + counter + " files of type '" + filter.getType() + "' with a total size of " + size + " bytes.\n");
  ...
  private static void specialFilesInFolder(final File file, final MyFileFilter filter)
  {
    ...
    System.out.println(file.getAbsolutePath() + " contains " + files.length + " files of type '" + filter.getType() + "' with a total size of " + counter + " bytes.\n");
  }

  private static boolean getRecursive(final String[] args)
  {
    for (int i = 0; i < args.length; i++)
      if (args[i].equalsIgnoreCase("-recursive")) return true;
    return false;
  }
  ...
  {
    for (int i = 0; i < args.length; i++)
      if (args[i].equalsIgnoreCase("-type") && i < args.length - 1) return args[i + 1];
    return null;
  }
  ...
  private static void fileAndSizeInFolder(final File file)
  {
    ...
    System.out.println(file.getAbsolutePath() + " contains " + files.length + " files with" + " a total size of " + counter + " bytes.\n");
  }
}