/*
  * ThreadUtil is a helper class that can be used to get a better overview of
  * threads in Java. The class can be used for two different things:
  *
  *
  * (1) SHOWING WHICH THREADS ARE PRINTING WHAT TO SYSTEM.OUT AND SYSTEM.ERR:
  *
  * The ThreadUtil.printThreadNames() method modifies System.out and System.err, so
  * the methods print and println prepend the name of the thread to what is printed.
  * For example, a thread with the name "MyThread" calls the method System.out.println("Hello"),
  * prints: "[MyThread] Hello".
  *
  * See also the demo class: threadutil.demo1.WhichThreadIsPrintingWhat.
  *
  *
  * (2) DISPLAY OF THE PROGRESS OF THE THREAD IN THE PROGRAM OVER TIME:
  *
  * The method ThreadUtil.showInvestigator(), produces a graphical window where
  * you can see how the thread state has been at all the times when
  * the associated method called ThreadUtil.makeSnapShot( String snapShotName )
  * So, first you call showInvestigator, then makeSnapShot
  * in all the places in your code where you would subsequently like to know
  * what the thread state looked like.
  */
...
/*
 * Investigator is a simple Facade to the ThreadInvestigator, and furthermore Investigator acts as single point of entry to the ThreadInvestigator, only giving access to one Thread at a time
 */
class Investigator {
