...
public class AccountConsumer extends Thread
{
  ...
  public void run()
  {
    ...
    while (true)
    {
      ...
      //Vi synkroniserer account objektet og sikrer, at koden i den synkroniserede blok kun kan køres af en tråd!
      synchronized (account)
      {
        ...
        else
        {
          try
          {
            System.out.println(getName() + ": Saldoen er mindre end " + amount + " kr., så jeg kalder account.wait()");
            account.wait();
          }
          catch (InterruptedException e)
          {
